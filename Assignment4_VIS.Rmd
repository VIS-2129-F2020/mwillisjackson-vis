...
iso_nola_french <- otp_isochrone(otpcon = otpcon, fromPlace = nola_french_performance,
                                 mode = "CAR", cutoffSec = 500) %>%
  st_transform(crs = LA_state_plane) %>%
  mutate(mode = "drive")

iso_6min_walk <- 
  otp_isochrone(otpcon = otpcon, fromPlace = NOLA_performance, 
                mode = "WALK", cutoffSec = 360) %>%
  st_transform(crs = LA_state_plane) %>%
  mutate(mode = "walk")

iso_6min_bike <- 
  otp_isochrone(otpcon = otpcon, fromPlace = NOLA_performance, 
                mode = "BICYCLE", cutoffSec = 360) %>%
  st_transform(crs = LA_state_plane) %>%
  mutate(mode = "bike")

iso_all_modes <- rbind(iso_6min_bike, iso_6min_walk)
...
right_side <- st_bbox(iso_nola_french)$xmax-700
left_side  <- st_bbox(iso_nola_french)$xmin+400
top_side <- st_bbox(iso_nola_french)$ymax-1100
...
ggplot(iso_areas,
  ...
  scale_x_continuous(name = 
                       "\nArea within a six minute walking distance\nof a performance venue (square km)") +
  scale_y_continuous(name = "Area within a six minute biking distance\nof a performance venue (square km)\n") +
  theme_economist()

```

```{r total acreage}
total_area_bike<- st_union(iso_6min_bike$geometry[iso_nola_french,])

total_area_walk<- st_union(iso_6min_walk$geometry[iso_nola_french,])

ggplot(total_area_bike) +
  geom_sf(data = nola_streets) +
  geom_sf() +
  geom_sf(data = total_area_walk, fill = "red") +
  geom_sf(data = NOLA_performance) +
  coord_sf(xlim = c(left_side, right_side), 
           ylim = c(bottom_side, top_side), expand = FALSE) +
  theme_map()
...
ggplot(total_area,
  ...
  scale_x_discrete(name = "Mode",
                   labels = c("By Biking","By Walking")) +
  geom_text(label = paste(prettyNum(total_area$area, digits = 3),"sq km"), 
            nudge_y = .4) +
  scale_fill_ordinal() +
  theme(legend.position = "none",
        panel.background = element_rect(fill = "white")) +
  labs(title = "Area Accessible Traveling Six Minutes from a Performance Venue in New Orleans")
