...
```{r, download geo data, message=F, comment='hide'}
...
```{r transform to same projection, message=F}
...
```{r additional variable changes (banksy variable),message=F}
...
```{r points near points, message=F}
...
```{r number and proportion of pubs in each neighborhood, message=F}
...
```{r density of points in polygons, message=F}
london_transform<- london_transform %>%
...
```{r openspace in each neighborhood (polygons), message=F}
london_transform<- london_transform %>%
...
```{r distance of pub to nearest openspace, message=F}
...
```{r avg distance between Banksy mural and its nearest pub, message=F, fig.height=5.8, fig.width=5.8}
