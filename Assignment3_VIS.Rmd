...
```{r, download geo data, message=F, comment='hide', warning=F}
...
```{r transform to same projection, message=F, comment='hide', warning=F}
...
```{r additional variable changes (banksy variable),message=F, comment='hide', warning=F}
...
```{r points near points, message=F, comment='hide', warning=F}
...
ggplot(london_transform) +
  ...
  geom_sf(data = speedcam2,
          aes(color = by_pub), size = 2) +
  scale_color_manual(values = c("black","red"),
                     name = "London Speedcams by \nDistance to a Pub",
                     labels = c("No pub within 500 m",
                                "Pub within 500m")) +
...
```{r number and proportion of pubs in each neighborhood, message=F, comment='hide', warning=F}
...
```{r density of points in polygons, message=F, comment='hide', warning=F}
london_transform<- london_transform %>%
...
```{r openspace in each neighborhood (polygons), message=F, comment='hide', warning=F}
london_transform<- london_transform %>%
...
ggplot(london_transform) +
  ...
  scale_fill_manual(values = c("lightgrey","seagreen4"),
...
```{r distance of pub to nearest openspace, message=F, comment='hide', warning=F}
...
ggplot(openspace_transform) +
  geom_sf(fill = "darkseagreen2", color = NA) +
...
```{r avg distance between Banksy mural and its nearest pub, message=F, comment='hide', warning=F, fig.height=5.8, fig.width=5.8}
...
banksy_map<- ggplot(london_transform) +
  ...
  geom_point(data = banksy_transform, aes(x = x, y = y, color = pub_dist), size = 25, shape = 15, alpha = .7) +
