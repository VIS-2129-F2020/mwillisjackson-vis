...
kings_data<- inner_join(race_data, age_data, by = "GEOID") %>%
  mutate(pct_not_white = (race_totalE - whiteE) / race_totalE,
         majority_not_white = pct_not_white > .5) %>%
  select(GEOID, pct_not_white, majority_not_white, median_ageE)

kings_data<- inner_join(kings_data, income_data, by = "GEOID") %>%
  select(GEOID, pct_not_white, majority_not_white, median_ageE, median_incomeE)

kings_data<- inner_join(kings_data, maj_ancestry, by = "GEOID") %>%
  select(GEOID, pct_not_white, majority_not_white, median_ageE, median_incomeE, maj_anc_type)
```
